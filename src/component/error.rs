...
impl fmt::Display for ComponentError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::TypeNotFound(type_id) => write!(f, "Component type {type_id:?} not found"),
            Self::PropsMismatch { expected, got } => write!(
                f,
                "Props type mismatch - expected {expected:?}, got {got:?}"
            ),
            ...
            Self::InvalidLifecycleTransition(phase, operation) => write!(
                f,
                "Invalid lifecycle transition: cannot {operation} while in {phase:?} phase"
            ),
