...
use crate::{
    component::{
        update_scheduler::{UpdatePriority, UpdateScheduler},
        AnyComponent, ComponentId, LifecyclePhase,
    },
    events::EventEmitter,
...
impl Context {
    ...
    pub fn request_update(&self, component_id: ComponentId) -> Result<(), String> {
        self.update_scheduler
            .schedule_update(component_id, UpdatePriority::Normal)
    }
    ...
    pub fn request_critical_update(&self, component_id: ComponentId) -> Result<(), String> {
        self.update_scheduler
            .schedule_update(component_id, UpdatePriority::Critical)
    }
...
mod tests {
    ...
    impl AnyComponent for TestComponent {
        ...
        fn any_before_update(&mut self, _props: Box<dyn Props>) -> Result<(), ComponentError> {
            self.lifecycle_events.push("before_update".to_string());
            Ok(())
        }

        fn any_update(&mut self, _props: Box<dyn Props>) -> Result<(), ComponentError> {
        ...
        fn any_on_mount(
            &mut self,
            _context: &crate::component::MountContext,
        ...
        fn any_on_update(
            &mut self,
            _changes: &crate::component::state_tracking::StateChanges,
        ) -> Result<(), ComponentError> {
        ...
        fn any_on_unmount(
            &mut self,
            _context: &crate::component::UnmountContext,
        ) -> Result<(), ComponentError> {
    ...
    fn test_context_lifecycle_hooks() {
        ...
        context.register_lifecycle_hook(LifecyclePhase::Unmounting, |component| {
            ...
            if let Some(test_component) = any.downcast_mut::<TestComponent>() {
                test_component
                    .lifecycle_events
                    .push("unmounting_hook".to_string());
            }
        });
        ...
        assert!(component
            .lifecycle_events
            .contains(&"mounting_hook".to_string()));
        ...
        assert!(component
            .lifecycle_events
            .contains(&"unmounting_hook".to_string()));
    }
