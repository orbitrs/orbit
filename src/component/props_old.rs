...
macro_rules! define_props {
        ...
        paste::paste! {
            ...
            impl [<$name Builder>] {
                pub fn new() -> Self {
                    Self {
                        $(
                            $field_name: define_props!(@field_default $($default)? $(#[required])?),
                        )*
                    }
                }
    ...
    // Helper rules to process field defaults and required status
...
macro_rules! validate_field {
    ($props_type:ty, $field:ident, $condition:expr, $message:expr) => {
        ...
        impl $crate::component::props::PropValidator<$props_type> for FieldValidator<$props_type> {
            fn validate(&self, props: &$props_type) -> Result<(), $crate::component::props::PropValidationError> {
                if !$condition(&props.$field) {
                    return Err($crate::component::props::PropValidationError::InvalidValue {
