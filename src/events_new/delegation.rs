...
impl<E: Event + Clone> Clone for DelegatedEvent<E> {
    fn clone(&self) -> Self {
        Self {
...
pub struct EventDelegate {
    ...
    capturing_handlers: Arc<RwLock<HashMap<TypeId, Vec<Box<dyn Fn(&mut dyn Event, &EventPropagation) + Send + Sync>>>>>,
    
    /// Map of event type to callbacks registered for bubbling phase
    bubbling_handlers: Arc<RwLock<HashMap<TypeId, Vec<Box<dyn Fn(&mut dyn Event, &EventPropagation) + Send + Sync>>>>>,
    
    /// Map of event type to callbacks registered for just that target
    target_handlers: Arc<RwLock<HashMap<TypeId, Vec<Box<dyn Fn(&mut dyn Event, &EventPropagation) + Send + Sync>>>>>,
...
impl EventDelegate {
    ...
    pub fn capture<E: Event + 'static>(&self, handler: impl Fn(&mut DelegatedEvent<E>) + Send + Sync + 'static) {
        let type_id = TypeId::of::<E>();
        let mut handlers = self.capturing_handlers.write().unwrap();
        
        let boxed_handler: Box<dyn Fn(&mut dyn Event, &EventPropagation) + Send + Sync> = Box::new(move |e, prop| {
            if let Some(event) = e.as_any().downcast_ref::<E>() {
                let mut delegated = DelegatedEvent {
                    event: event.clone(),
                    propagation: prop.clone(),
                };
                handler(&mut delegated);
            }
        });
    ...
    pub fn bubble<E: Event + Clone + 'static>(&self, handler: impl Fn(&mut DelegatedEvent<E>) + Send + Sync + 'static) {
        let type_id = TypeId::of::<E>();
    ...
    pub fn on<E: Event + Clone + 'static>(&self, handler: impl Fn(&mut DelegatedEvent<E>) + Send + Sync + 'static) {
        let type_id = TypeId::of::<E>();
    ...
    pub fn dispatch<E: Event + Clone + 'static>(&self, event: &E, target_id: Option<usize>) {
    ...
    fn dispatch_capturing<E: Event + Clone + 'static>(&self, event: &E, propagation: &mut EventPropagation, target_id: Option<usize>) {
    ...
    fn dispatch_bubbling<E: Event + Clone + 'static>(&self, event: &E, propagation: &mut EventPropagation, target_id: Option<usize>) {
    ...
    fn handle_capturing_event<E: Event + Clone + 'static>(&self, event: &E, propagation: &EventPropagation) {
        let type_id = TypeId::of::<E>();
        if let Ok(handlers) = self.capturing_handlers.read() {
            if let Some(handlers) = handlers.get(&type_id) {
                let boxed_event = event.clone();
                for handler in handlers {
                    handler(&mut boxed_event.clone(), propagation);
                    if propagation.is_propagation_stopped() {
                        break;
                    }
                }
    ...
    fn handle_bubbling_event<E: Event + Clone + 'static>(&self, event: &E, propagation: &EventPropagation) {
        let type_id = TypeId::of::<E>();
    ...
    fn handle_event<E: Event + Clone + 'static>(&self, event: &E, propagation: &EventPropagation) {
        let type_id = TypeId::of::<E>();
...
pub fn build_delegation_tree(node: &Node, parent_delegate: Option<Arc<Mutex<EventDelegate>>>) -> Arc<Mutex<EventDelegate>> {
    let delegate = Arc::new(Mutex::new(EventDelegate::new(node.id())));
